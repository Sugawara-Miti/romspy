...
def read_osaka(csvfile):
    ...
    data = pd.read_csv(csvfile, parse_dates=[['date', 'hour']], date_parser=_parse, na_values='--' )
    data = data.interpolate()
    data = data.fillna(method='bfill')
    data['mjd'] = data['date_hour'].apply(lambda t: netCDF4.date2num(t, 'days since 1968-05-23 09:00:00 GMT') )
    print data.describe()
