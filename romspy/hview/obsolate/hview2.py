...
def hview2(ncfile, figfile=None, vname=None, t=-1, k=None, vmax=None, vmin=None,
    ...
    if vmax is not None:
        interval = np.arange(vmin, vmax, interval)
        CF = ax.contourf(X+0.00449, Y+0.00546, var2d, interval, cmap=cmap, vmax=vmax, vmin=vmin)
    else:
        interval = np.arange(np.min(var2d), np.max(var2d), interval)
        CF = ax.contourf(X+0.00449, Y+0.00546, var2d, interval, cmap=cmap)
