...
class Dataset():
    ...
    def hview(self, vname, time=-1, k=20, fmt='%i', cff=None, unit='g', 
        ...
        if vname == 'velocity':
            ax = self.add_quiver(vname, t, k, cff, step, scale)
        else:
            if 'pcolor' in method:
                ax = self.add_pcolor(vname, t, k, cff, levels, cblabel, fmt)
            if 'contour' in method:
                ax = self.add_contour(vname, t, k, cff, levels, cblabel, fmt)
            if 'fill' in method:
                ax = self.add_contourf(vname, t, k, cff, levels, cblabel, fmt)
    ...
    def add_pcolor(self, vname, t, k, cff, levels, cblabel, fmt):
        ...
        if levels is not None:
            PC = ax.pcolor(X, Y, var2d, vmin=levels[0], vmax=levels[-1])
        else:
            PC = ax.pcolor(X, Y, var2d)
    ...
    def add_contourf(self, vname, t, k, cff, levels, cblabel, fmt):
        ...
        ax = plt.gca()
        if levels is not None:
            CF = ax.contourf(X, Y, var2d, levels, extend='both')
        else:
            CF = ax.contourf(X, Y, var2d, extend='both')
        CB = plt.colorbar(CF)
...
if __name__ == '__main__':
    mapfile = 'F:/okada/Dropbox/Maps/deg_OsakaBayMap_okada.bln'
    
    #ncfile = 'Z:/apps/OB500_fennelP/4DVAR04/output/ob500_ini_0.nc'
    ncfile = 'Z:/apps/OB500P/case20/NL1/ob500_rst.nc'
